...
class Account_Info(Info_Interface):
    ...
    def init_info(self, client):
        ...
        ##########################################
        batch_size = 10
        symbol_batches = [exist_symbols[i:i + batch_size] for i in range(0, len(exist_symbols), batch_size)]

        # Iterate over batches and make API calls
        price_dfs = []
        for symbols_batch in symbol_batches:
            price_df_batch = pd.DataFrame(client.ticker_price(symbols=symbols_batch))
            price_dfs.append(price_df_batch)
    ...
    def update_info(self, client):
        ...
        # 更新价格
        ##########################################
        all_symbols = list(position_df.index.values)
        batch_size = 10
        symbol_batches = [all_symbols[i:i + batch_size] for i in range(0, len(all_symbols), batch_size)]
        price_lists = []
        for symbols_batch in symbol_batches:
            temp_price_list = client.ticker_price(symbols=symbols_batch)
            price_lists += temp_price_list
        price_df = pd.DataFrame(price_lists)
        ###########################################
